...
import pandas as pd 
from recomendador_itemaitem import item_item_recom
...
def index():                                                    # mensaje de inicio de la API
    return 'API desarrollada para el proyecto individual 1 de la carrera de ciencia de datos por Liz Mariana Henao Cardona'
...
def developer(desarrollador: str):
    ...
    df = pd.read_parquet('desarrollador.parquet')      # cargamos dataframe
...
def userdata(user_id: str):
    ...
    df = pd.read_parquet('userdata.parquet')               # cargamos dataframe desde archivo
...
def UserForGenre(genero: str):
    ...
    try:
        df = pd.read_parquet('userporgenero.parquet')
...
def developer_reviews_analysis(desarrolladora: str):
    ...
    df = pd.read_parquet('analisisreseñasdesarrolladores.parquet')
